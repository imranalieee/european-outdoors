...
import { GetItemsForOrderItemPack, SetOrderState, AddOrderItemsToPack } from '../../../../../redux/slices/order/order-slice';
...
const AddItems = ({ isOpen, onClose, orderItemId }) => {
  ...
  const getItemsForOrderItemPack = () => {
    const skip = (packPageNumber - 1) * packPageLimit;
    const limit = packPageLimit;

    dispatch(GetItemsForOrderItemPack({
      skip, limit, filters: packFilters, orderItemId, sortBy: sortValue
    }));
  };

  const handleAddItemsToPack = () => {
    if (addItemsToPackList.length) {
      dispatch(AddOrderItemsToPack({ orderItemId, items: addItemsToPackList, action: 'addItemsToPack' }));
    }
  };
  ...
  useEffect(() => {
    if (orderItemId) getItemsForOrderItemPack();
  }, [packFilters, packPageNumber, packPageLimit, sortValue, orderItemId]);
  ...
  useEffect(() => () => {
    ...
    dispatch(SetOrderState({ field: 'totalItemsForPack', value: 0 }));
  }, []);

  useEffect(() => {
    if (orderPackItemAdded) {
      onClose();
      handleCloseAddItemsToPack();
      dispatch(SetOrderState({ field: 'orderPackItemAdded', value: false }));
    }
  }, [orderPackItemAdded]);
  ...
  return (
    <Drawer open={isOpen} width="1144px" close={() => { onClose(); handleCloseAddItemsToPack(); }}>
      ...
      <Box mt={3.125}>
        {getOrderItemsForPackLoading || addOrderPackItemLoading ? <LoaderWrapper /> : null}
        <Table
        ...
        >
          {getOrderItemsForPackLoading ? <LoaderWrapper /> : null}
